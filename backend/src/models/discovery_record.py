...
class DiscoveryRecord(Base):
    ...
    # Research metadata
    source = Column(String(255), nullable=True)   # "Google Search", "LinkedIn", etc.
    status = Column(String(50), nullable=True)   # "Verified", "Pending", etc.
    
    # Intelligence data (stored as JSON)
    industry = Column(String(255), nullable=True)
    employee_count = Column(String(100), nullable=True)
    technology_stack = Column(JSON, nullable=True)   # List of technologies
    business_activity = Column(Text, nullable=True)
    
    # Additional metadata
    research_data = Column(JSON, nullable=True)   # Full research response
    location_data = Column(JSON, nullable=True)   # Location/coordinates if available
