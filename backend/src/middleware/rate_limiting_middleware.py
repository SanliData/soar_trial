...
# In-memory rate limit storage (for P0; use Redis in production)
...
RATE_LIMITS = {
    ...
    "api_key": (1000, 60),   # 1000 requests per 60 seconds for API key
}


class RateLimitingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        # Skip rate limiting in development mode or if explicitly disabled
        if ENV != "production" or DISABLE_RATE_LIMIT:
            return await call_next(request)
        
        # Skip rate limiting for health checks and static UI files
